...
describe('Verifica os retornos da função buyStocks na camada de CONTROLLER', () => {
  ...
  it('Deve retornar um JSON com a mensagem "Successful purchase" e o transactionId', async () => {
    await controller.buyStocks(request, response);
    expect(response.json.calledWith(mocks.buyMade)).to.be.true;
  });
});

describe('Verifica os retornos da função sellStocks na camada de CONTROLLER', () => {
  const response = { locals: { payload: { customerId: 1 } } };
  const request = { body: { stockId: 1, quantity: 1 } };
  ...
  beforeEach(() => {
    ...
    sinon.stub(service, 'sellStocks').resolves(mocks.saleMade);
  });

  afterEach(() => {
    service.sellStocks.restore();
  });

  it('O status retornado deve ser 200', async () => {
    await controller.sellStocks(request, response);
    expect(response.status.calledWith(200)).to.be.true;
  });

  it('Deve retornar um JSON com a mensagem "Successful sale" e o transactionId', async () => {
    await controller.sellStocks(request, response);
    expect(response.json.calledWith(mocks.saleMade)).to.be.true;
  });
});
